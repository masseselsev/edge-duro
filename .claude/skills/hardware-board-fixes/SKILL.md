...
persistent cross-build cache -- see `_fetch_opi5_plus_firmware` in
`core/workspace.py`.
...
| Installing a whole firmware/driver package for one file | Check size first (`Installed-Size` in the repo's `Packages` index) -- `linux-firmware` is ~655 MB download for one ~3 KB blob. If the file has a stable upstream home (kernel.org/gitlab.com project, not just a distro package), fetch it directly instead of through apt -- see `_fetch_opi5_plus_firmware` in `core/workspace.py` |
