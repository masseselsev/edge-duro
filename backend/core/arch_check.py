...
def fetch_index_text(url: str, suite: str, component: str, arch: str) -> Optional[str]:
...
def _available_names(
    ...
    for url, suite, component in sources:
        ...
        if text is None:
            unreachable.append(f"{url} {suite}/{component} [{arch}]")
            continue
...
def check_recipe_packages(recipe, log=print, fetch=None) -> ArchCheckResult:
    ...
    sources = list(official_index_sources(recipe.distribution, recipe.release, recipe.architecture))
    ...
    available, unreachable = _available_names(sources, arch, fetch, log)
    ...
    if unreachable:
        ...
        for source in unreachable:
            log(f"[ARCH CHECK WARNING] Package index unreachable: {source}")
        log("[ARCH CHECK] Architecture availability check skipped -- at least one package index could not be read.")
        return ArchCheckResult(checked=False, missing=[], unreachable=unreachable)
